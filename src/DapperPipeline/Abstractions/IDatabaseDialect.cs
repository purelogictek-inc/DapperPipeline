...
public interface IDatabaseDialect
{
    ...
    /// <summary>
    /// Renders the alignment marker emitted after a command that registered result readers.
    /// </summary>
    /// <remarks>
    /// <para>
    /// A batch returns an anonymous ordered stream of result sets — nothing in the wire protocol
    /// says which statement produced which. The pipeline therefore pairs readers to result sets by
    /// position, which is correct only while every command yields exactly as many row-returning
    /// statements as it registered readers. The marker puts the boundary <em>into</em> the stream so
    /// the pipeline can verify that instead of assuming it: after a command's readers have run, the
    /// next result set must be that command's token, or its results and the next command's have
    /// crossed.
    /// </para>
    /// <para>
    /// The default is valid on PostgreSQL, SQLite and SQL Server. <paramref name="token"/> is
    /// generated by the pipeline from a fixed charset (<c>dpm_</c> plus digits) and never contains
    /// caller input, so it is safe to inline — but a dialect overriding this must keep that true.
    /// </para>
    /// </remarks>
    string RenderAlignmentMarker(string token) => $"SELECT '{token}' AS __dp_marker";
